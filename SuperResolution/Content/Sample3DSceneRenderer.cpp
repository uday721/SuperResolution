...
using namespace SuperResolution;
...
hstring const& AngleKey = L"Angle";
hstring const& TrackingKey = L"Tracking";
...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	// Create a root signature with a single constant buffer slot.
	{
		...
		com_ptr<ID3DBlob> pError;
		DX::ThrowIfFailed(D3D12SerializeRootSignature(&descRootSignature, D3D_ROOT_SIGNATURE_VERSION_1, pSignature.put(), pError.put()));
		DX::ThrowIfFailed(d3dDevice->CreateRootSignature(0, pSignature->GetBufferPointer(), pSignature->GetBufferSize(), IID_PPV_ARGS(m_rootSignature.put())));
	...
	auto createPipelineStateTask = (createPSTask && createVSTask).then([this]() {
		...
		DX::ThrowIfFailed(m_deviceResources->GetD3DDevice()->CreateGraphicsPipelineState(&state, IID_PPV_ARGS(m_pipelineState.put())));
	...
	auto createAssetsTask = createPipelineStateTask.then([this]() {
		...
		DX::ThrowIfFailed(d3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, m_deviceResources->GetCommandAllocator(), m_pipelineState.get(), IID_PPV_ARGS(m_commandList.put())));
		...
		// Create a descriptor heap for the constant buffers.
		{
			...
			DX::ThrowIfFailed(d3dDevice->CreateDescriptorHeap(&heapDesc, IID_PPV_ARGS(m_cbvHeap.put())));
