...
void DX::DeviceResources::CreateDeviceResources()
{
	...
	DX::ThrowIfFailed(CreateDXGIFactory1(IID_PPV_ARGS(m_dxgiFactory.put())));
	...
	if (FAILED(hr))
	{
		...
		DX::ThrowIfFailed(m_dxgiFactory->EnumWarpAdapter(IID_PPV_ARGS(warpAdapter.put())));

		hr = D3D12CreateDevice(warpAdapter.get(), D3D_FEATURE_LEVEL_11_0, IID_PPV_ARGS(m_d3dDevice.put()));
	}
#endif
	...
	DX::ThrowIfFailed(m_d3dDevice->CreateCommandQueue(&queueDesc, IID_PPV_ARGS(m_commandQueue.put())));
	...
	DX::ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&rtvHeapDesc, IID_PPV_ARGS(m_rtvHeap.put())));
	...
	ThrowIfFailed(m_d3dDevice->CreateDescriptorHeap(&dsvHeapDesc, IID_PPV_ARGS(m_dsvHeap.put())));
	NAME_D3D12_OBJECT(m_dsvHeap);

	for (UINT n = 0; n < c_frameCount; n++)
	{
		DX::ThrowIfFailed(
			m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(m_commandAllocators[n].put()))
			);
	}

	// Create synchronization objects.
	DX::ThrowIfFailed(m_d3dDevice->CreateFence(m_fenceValues[m_currentFrame], D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(m_fence.put())));
...
void DX::DeviceResources::CreateWindowSizeDependentResources()
{
	...
	else
	{
		...
		//winrt::check_hresult(swapChain.as(swapChain.put()));
	}
	...
	// Create render target views of the swap chain back buffer.
	{
		...
		for (UINT n = 0; n < c_frameCount; n++)
		{
			DX::ThrowIfFailed(m_swapChain->GetBuffer(n, IID_PPV_ARGS(m_renderTargets[n].put())));
...
void DX::DeviceResources::SetDpi(float dpi)
{
	if (dpi != m_dpi)
	{
		...
		m_logicalSize = Windows::Foundation::Size(m_window.get().Bounds().Width,m_window.get().Bounds().Height);
...
void DX::DeviceResources::ValidateDevice()
{
	...
	DXGI_ADAPTER_DESC currentDesc;
	{
		...
		DX::ThrowIfFailed(CreateDXGIFactory1(IID_PPV_ARGS(currentDxgiFactory.put())));
...
void DX::DeviceResources::GetHardwareAdapter(IDXGIAdapter1** ppAdapter)
{
	...
	*ppAdapter = nullptr;
	...
	for (UINT adapterIndex = 0; DXGI_ERROR_NOT_FOUND != m_dxgiFactory->EnumAdapters1(adapterIndex, adapter.put()); adapterIndex++)
	{
